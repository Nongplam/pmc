...
$query="select dailysaledetail.*,product.pname,dailysalemaster.moneyreceive,dailysalemaster.moneychange,dailysalemaster.refkey,dailysalemaster.billno,subbranch.tel from dailysaledetail,product,stock,dailysalemaster,subbranch where dailysaledetail.stockid=stock.sid AND stock.productid=product.regno AND dailysaledetail.masterid=dailysalemaster.dmid AND dailysaledetail.subbranchid = subbranch.id AND masterid='$dmidtemp'";
...
if(mysqli_num_rows($result)>0) {
    ...
    $res[] = array('stockid' => $rows['stockid'],'qty'=>$rows['qty'],
                  'price'=>$rows['price'],'masterid'=>$rows['masterid'],'date'=>$rows['date'],'subbranchid'=>$rows['subbranchid'],'pname'=>$rows['pname'],'moneyreceive'=>$rows['moneyreceive'],'moneychange'=>$rows['moneychange'],'refkey'=>$rows['refkey'],'billno'=>$rows['billno'],'tel'=>$rows['tel']);
    }
